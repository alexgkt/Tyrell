...
### Recomendations
Make sure these columns drom this tables are indexed propertly. Mostly are used in either `JOIN` `LIKE` `GROUP BY` `SORT BY` statements.
```
jobs.id
jobs.name
jobs.description
...
jobs_personalities.job_id
jobs_personalities.name

practical_skills.id
practical_skills.deleted

jobs_basic_abilities.job_id
...
jobs_career_paths.job_id
jobs_career_paths.name

jobs_rec_qualifications.id
jobs_rec_qualifications.name
jobs_rec_qualifications.job_id
...
job_types.deleted
```
Consider replacing all `LIKE '%キャビンアテンダント%'` conditions with MYSQL full text search instead for better performance as using `LIKE` with `%` wildcard in front is a known performance hogger.
